...
def _normalise_mask(image: np.ndarray, path: Path) -> np.ndarray:
    if image.ndim == 3:
        if image.shape[2] == 3:
...
def _track_frames(
    ...
    for frame_idx, detections in frames:
        ...
        lines.append(_format_line(frame_idx, assignments))
        target_total += len(assignments)
        if assignments:
            maximum_track_id = max(
                maximum_track_id,
                max(track_id for track_id, _ in assignments),
            )
    return lines, target_total, maximum_track_id
        ...
        if input_mode == "txt":
            ...
            frames = [
                (
                    frame_idx,
                    _extract_detections(
                        mask_path, args.threshold, args.min_area
                    ),
                )

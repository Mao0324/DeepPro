...
class detector(nn.Module):
    def __init__(self, num_classes, seqlen=100, out_len=100):
        ...
        self.layer1 = nn.Sequential(self.pool, STD_Resblock(8, 16), STD_Resblock(16, 32))
        self.layer2 = nn.Sequential(self.pool, STD_Resblock(16, 32))
        self.TATT1 = TPro(d_model=32, num_head=8, seqlen=seqlen, out_len=out_len)
        self.conv_out1_1 = nn.Sequential(nn.Conv3d(in_channels=32, out_channels=8, kernel_size=(1,1,1), stride=(1,1,1), padding=(0,0,0)),
                                       nn.BatchNorm3d(8), nn.ReLU(inplace=True))
        self.TATT2 = TPro(d_model=32, num_head=8, seqlen=seqlen, out_len=out_len)
        self.conv_out1_2 = nn.Sequential(nn.Conv3d(in_channels=32, out_channels=8, kernel_size=(1,1,1), stride=(1,1,1), padding=(0,0,0)),
                                       nn.BatchNorm3d(8), nn.ReLU(inplace=True))
        self.conv_out2 = nn.Sequential(nn.Conv3d(in_channels=8*2, out_channels=8, kernel_size=(1,1,1), stride=(1,1,1), padding=(0,0,0)),
                                       nn.BatchNorm3d(8), nn.ReLU(inplace=True))
    ...
    def forward(self, seq_imgs):  ## 1.415G
        ...
        seq_feats = self.layer1[:2](seq_feats)
        seq_feats1 = self.layer1[2](seq_feats)  ## 20.771G
        ...
        seq_feats = self.conv_out2(torch.cat([seq_feats1, seq_feats2], dim=1))
...
class HAMloss(nn.Module):
    def __init__(self, alpha=[0.1667, 0.8333], gamma=2, MaxClutterNum=39, ProtectedArea=2):
        ...
        self.HAM = HAM_loss(gamma=gamma, alpha=alpha, MaxClutterNum=MaxClutterNum, ProtectedArea=ProtectedArea)
    ...
    def forward(self, midpred, target):
        ...
        loss_mid = self.HAM(input, target)

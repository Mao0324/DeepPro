...
screen -dmS "$SESSION" -L -Logfile "$SCREEN_LOG" \
    env \
        ...
        FEEDBACK_TRAIN_AMP="$FEEDBACK_TRAIN_AMP" \
        FEEDBACK_EPOCHS="${FEEDBACK_EPOCHS:-100}" \
        FEEDBACK_EVAL_INTERVAL="${FEEDBACK_EVAL_INTERVAL:-5}" \
        FEEDBACK_STEP_SIZE="${FEEDBACK_STEP_SIZE:-10}" \
        FEEDBACK_EARLY_STOPPING_START="${FEEDBACK_EARLY_STOPPING_START:-20}" \
        FEEDBACK_F1_WARMUP_EPOCHS="${FEEDBACK_F1_WARMUP_EPOCHS:-5}" \
        FEEDBACK_F1_RAMP_EPOCHS="${FEEDBACK_F1_RAMP_EPOCHS:-10}" \
...
echo "train_amp=$FEEDBACK_TRAIN_AMP"
echo "resume_preserves_active_schedule=epochs:${FEEDBACK_EPOCHS:-100} step:${FEEDBACK_STEP_SIZE:-10} f1_warmup:${FEEDBACK_F1_WARMUP_EPOCHS:-5} f1_ramp:${FEEDBACK_F1_RAMP_EPOCHS:-10}"

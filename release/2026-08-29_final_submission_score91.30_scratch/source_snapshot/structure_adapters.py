...
class RawAppearanceMotionAdapter(nn.Module):
    ...
    def forward(self, backbone_features, raw_frames, return_aux=False):
        ...
        if not return_aux:
            # These full-resolution tensors are only needed in the auxiliary
            # training output. Releasing them before the three-branch fusion
            # avoids retaining several gigabytes during large-image inference
            # without changing any computation or model parameter.
